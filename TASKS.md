...
- [x] 文件上传与解析（PDF/DOCX/PPTX/XLSX/TXT/MD/HTML）、文本切分、嵌入配置（可选向量，降级关键词）、文件问答与引用、FTS5+LIKE 全局搜索、自定义字段、CSV 人物导入/Markdown 文件解析、长对话压缩。

### P2 社交能力版本

- [x] 聊天前简报、互动信息自动提取与用户确认、聊天复盘、AI 模拟聊天与多维度评分、社交目标（user_profiles）、间隔复习、每周成长报告、长期记忆审批、待跟进提醒闭环。
