...
> 建议节奏：P0 约 3–5 周（单人全职）；P1 约 3–4 周；P2 约 3–4 周；P3 按模块拆分、不设统一截止。

---

## 4. P0 任务分解总览

P0 按 7 个里程碑（M0–M7）推进，逐项验收，详见 [TASKS.md](./TASKS.md)：

| 里程碑 | 内容 | 关键交付 |
|---|---|---|
| M0 初始化 | 仓库、原生环境脚本、后端骨架、前端骨架、联调冒烟 | 一条命令启动、健康检查 |
| M1 数据库 | Alembic 迁移（SQLite）、P0 全部表、种子数据、Repository 基座 | 迁移可升降级 |
| M2 人物与互动 | 人物/标签/事实/时间线/互动 CRUD + 前端 | 建人→加标签→记互动→看时间线 |
| M3 话题知识库 | 分类/话题/笔记 CRUD、Tiptap 编辑器、自动保存 | Notion 式笔记可用 |
| M4 AI 提供商 | 主密码与加密、提供商 CRUD、适配器、模型同步 | 配置 DeepSeek 并连通 |
| M5 AI 对话 | 对话 CRUD、SSE 流式、停止/重新生成、上下文组装、关联面板 | 流式对话可保存历史 |
| M6 备份与导出 | JSON 导入导出、SQLite 备份/恢复、Markdown 导出 | 备份→清库→恢复成功 |
| M7 收尾 | README、端到端手测、安全自查、最终验收 | 对照 17.1 全部通过 |

依赖关系：M0 → M1 →（M2、M3 可并行）→ M4 → M5 → M6 → M7。M2/M3 依赖 M1 完成。
